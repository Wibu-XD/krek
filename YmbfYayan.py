...
koh = '100005395413800'
hoetank = random.choice(['Arya Ganteng^_^', 'Wibu Pepek', 'WUHAN KONTOL'])
...
# LO KONTOL
logo = ''' SEBUAH TOOLS UNTUK NGEKREK
Recode:Arya-XD
Facebook:Muhammad Arya Adinata
WhatsApp:089694295787'''

Arya Ganteng = '1714009362122228'
# crack selesai
def Hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '\n\n [%s!%s] kamu tidak mendapatkan hasil,karna lo wibu yahahah :('%(M,N);exit()
...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s*%s ohayou onii-chan %s%s%s'%(O,N,K,nama,N);time.sleep(2)
...
def moch_yayan():
    ...
    print ' (\033[0;96m•\033[0m) ORANG GANTENG : %s'%(nama)
...
def info_tools():
    ...
    print '\n %s[%s>%s] Yt       : PukiPie'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Recode   : Arya-XD'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : -'%(N,H,N);time.sleep(0.07)
    ...
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/arya.d6x'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Blogspot : -'%(N,H,N);time.sleep(0.07)
